...
function printHousesOrHouses<T extends Printable>(arr: T[]):void {
    for (let i = 0; i < arr.length; i++) {
        arr[i].print()       
    }
}

printHousesOrHouses<House>([new House(), new House()])
printHousesOrHouses<Car>([new Car(), new Car()])